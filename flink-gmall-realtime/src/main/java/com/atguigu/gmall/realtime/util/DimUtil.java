...
public class DimUtil {
    ...
    /**
     * 查询维度数据优化：旁路缓存
     * 先从Redis中查询维度数据，如果查询到，那么直接返回；如果在Redis中没有查询到维度数据，
    ...
    public static JSONObject getDimInfo(Connection conn, String tableName, Tuple2<String, String>... columnNameAndValues) {
        ...
        if (dimJsonStr != null && dimJsonStr.length() > 0) {
            ...
            // 在Redis中没有命中缓存，发送请求到phoenix表中进行查询
            System.out.println("从phoenix表中查询维度的sql:" + selectSql);

            // 底层还是调用 PhoenixUtil，从phoenix表中进行查询
            List<JSONObject> dimList =
                    PhoenixUtil.queryList(conn, selectSql.toString(), JSONObject.class);
            ...
            if (dimList != null && dimList.size() > 0) {
                ...
                if (jedis != null) {
                    jedis.setex(redisKey.toString(), 3600 * 24,
                            dimJsonObj.toJSONString());
                }
            } else {
